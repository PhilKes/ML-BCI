...
class PHYS:
    ...
    def set_rests_config(from_bl_run=None, less_rests=None):
        if from_bl_run is not None:
            PHYS.set_rest_from_bl_run(from_bl_run)
        if less_rests is not None:
            PHYS.set_rest_trials_less(less_rests)

    @staticmethod
    def set_rest_from_bl_run(val):
        PHYS.CONFIG.REST_TRIALS_FROM_BASELINE_RUN = val

    @staticmethod
    def set_rest_trials_less(val):
        PHYS.CONFIG.REST_TRIALS_LESS = val
    ...
    ALL_SUBJECTS = [i for i in range(1, 110) if i not in [88, 92, 100, 104]]
