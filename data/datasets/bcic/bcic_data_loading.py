...
from config import DATA_PRELOAD, BATCH_SIZE, global_config
...
class BCIC_Dataloader(MI_DataLoader):
    ...
    def create_loaders_from_splits(splits, validation_subjects, n_class, device, preloaded_data=None,
        ...
        if len(validation_subjects) > 0:
            ...
            loader_valid = BCIC_Dataloader.create_loader_from_subjects(validation_subjects, n_class, device,
                                                                       preloaded_data[subjects_valid_idxs, :, :, :],
                                                                       preloaded_labels[subjects_valid_idxs, :],
                                                                       bs, ch_names, equal_trials)

        loader_train = BCIC_Dataloader.create_loader_from_subjects(subjects_train, n_class, device,
                                                                   preloaded_data[subjects_train_idxs, :, :, :],
                                                                   preloaded_labels[subjects_train_idxs, :],
                                                                   bs, ch_names, equal_trials)
        loader_test = BCIC_Dataloader.create_loader_from_subjects(subjects_test, n_class, device,
    ...
    @staticmethod
    def create_preloaded_loader(subjects_chunk, n_class, ch_names, batch_size, device, equal_trials):
        # TODO
        raise NotImplementedError('This method is not implemented!')

    @staticmethod
    def mne_load_subject_raw(subject, runs, ch_names=[], notch=False, fmin=global_config.FREQ_FILTER_HIGHPASS,
                             fmax=global_config.FREQ_FILTER_LOWPASS):
        # TODO
        raise NotImplementedError('This method is not implemented!')

    """
    Subroutine:  load_subjects_data()
    """
