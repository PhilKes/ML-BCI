...
from app.util.misc import save_dataframe, print_pretty_table
...
def slice_trials(data: np.ndarray, labels: np.ndarray, slices: int):
    ...
    # logging.info(collections.Counter(labels))
    # logging.info(collections.Counter(labels_split))
...
def get_correctly_predicted_areas(n_class, sample_predictions, trials_classes, trials_start_samples, max_samples):
    ...
    for idx, trial_class in enumerate(trials_classes):
        ...
        # logging.info(f"Trial {idx:02d}: {trials_correct_areas_relative[idx]:.3f}")
