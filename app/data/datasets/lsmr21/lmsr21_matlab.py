...
from app.util.misc import copy_attrs, to_el_list, print_pretty_table, print_counts
...
class LSMRSubjectRun:
    ...
    def get_data(self, trials_idxs: List[int] = None, min_trial_time=None, ch_idxs=range(len(LSMR21.CHANNELS))):
        ...
        # logging.info("Slicing Time: %s", f"{time.time() - start:.2f}")
        return data
    ...
    def get_trials(self, n_class=4, min_trial_time=CONFIG.EEG.TMIN, ignore_target=None, artifact=-1, trial_category=-1):
        ...
        # logging.info("n-class Trials: %s",len(trials))
    ...
    def print_trials_with_min_mi_time(self, mi_tmins=[4, 5, 6, 7, 8, 9, 10, 11]):
        ...
        """
        logging.info(f"-- Subject {self.subject} Run {self.trialdata[0].runnumber} "
    ...
    def print_stats(self):
        ...
        # logging.info("NaN results: "+str(np.count_nonzero(np.isnan(results))))
        logging.info("--- Task Nrs (in blocks of 75) ---")
        print_counts(tasknrs)
        logging.info("--- Total Trials ---")
        logging.info(str(len(results))+"\n")
        logging.info("--- Targets---")
        print_counts(targets)
        logging.info("--- Result Counts ---")
        print_counts(results)
        logging.info("--- Forcedresult Counts ---")
        print_counts(forcedresults)
        logging.info("--- Samples Counts ---")
        logging.info(f"min: {np.min(samples)} max: {np.max(samples)}")
