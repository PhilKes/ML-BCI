...
from app.data.data_utils import get_trials_size, get_runs_of_n_class, get_data_from_raw, map_trial_labels_to_classes, \
    map_times_to_samples, dec_label, get_equal_trials_per_class, increase_label
...
class PHYSDataLoader(MIDataLoader):
    ...
    def load_subjects_data(cls, subjects: List[int], n_class: int, ch_names: List[str] = PHYS.CHANNELS,
        ...
        for i, subject in TqdmProgressBar(enumerate(subjects), total=len(subjects)):
    ...
    def mne_load_rests(cls, subject: int, trials: int, ch_names: List[str], samples: int):
        ...
        if trials_diff > 0:
            for m in range(trials_diff):
                ...
                # logging.info("rand_start %s", rand_start_idx)
        ...
        # logging.info("X %s Y %s", X.shape, y)
        X = np.swapaxes(X, 2, 1)
        return X, y
    ...
    def mne_load_subject_raw(cls, subject: int, runs: List[int], ch_names: List[str] = PHYS.CHANNELS):
        ...
        """
        if VERBOSE:
            logging.info(f"MNE loading Subject {subject} Runs {runs}")
