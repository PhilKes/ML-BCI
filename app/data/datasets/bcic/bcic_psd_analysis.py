...
def calc_psds():
    ...
    logging.info("  - Subjects: %s", subjects)
    logging.info("  - n_class: %s", n_class)

    logging.info('  - Load BCIC data set')
    equal_trials = True
    ...
    logging.info("  - Calculate PSDs")
    BCIC_IV2a_dataset.calc_psds(n_class, subjects, data, labels, path=f"{results_folder}/psds_BCIC")
    logging.info("  - PSDs are save in files!")


########################################################################################
if __name__ == '__main__':
    logging.info("PSD analysis of BCIC data set started with following parameters:")
    # calc_psds() only has to be called if one of the parameters defined therein has been changed !!!
    calc_psds()

    logging.info(' Read PSDs from files and plot them ')
    plot_psds()
