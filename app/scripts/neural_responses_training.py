...
from app.data.data_utils import subtract_first_config_accs, save_accs_panda
...
from app.util.misc import datetime_to_folder_str
...
for tmin in tmins:
    ...
    for ds_idx, ds in enumerate(ds_used):
        for fb_idx, fb_name in enumerate(FBS_NAMES):
            logging.info(f'Training with tmin={tmin}, tmax={tmax} for {fb_name} with {ds}')
...
logging.info(f"Executing Training for Neural Response Frequency bands")
