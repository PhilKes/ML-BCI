...
from app.paths import benchmark_results_folder
from app.scripts.visualize_bench_all import visualize_benchmarks
...
if (len(args.batch_size) < 1) | any(bs < 1 for bs in args.batch_size):
    parser.error("Batchsizes (--batch_size) must be a list of integers > 0")
...
batch_lat_avgs, trial_inf_time_avgs = np.zeros((len(all_confs), len(args.batch_size), len(args.n_classes))), np.zeros(
    (len(all_confs), len(args.batch_size), len(args.n_classes)))

# Run all Configurations with all Batchsizes
for i, conf in enumerate(all_confs):
    logging.info(f"Conf {i} {conf}")
    for bs_idx, bs in enumerate(args.batch_size):
...
np.savez(f"{parent_folder}/results.npz",
         batch_sizes=np.array(args.batch_size, dtype=np.int),
