...
from app.machine_learning.configs_results import create_results_folders, save_config, training_config_str, \
    save_n_class_results
from app.machine_learning.modes import do_n_class_training_cv
from app.machine_learning.util import preferred_device, overlapping_trials_slicing
from app.util.dot_dict import DotDict
from app.util.misc import datetime_to_folder_str, print_numpy_counts, groups_labels
...
def trials_sliced_training(argv=sys.argv[1:]):
    ...
    logging.info(preloaded_data.shape)
    ...
    logging.info("Sliced Shape:")
    logging.info(preloaded_data.shape)
    logging.info("Sliced Labels Stats Subject1:")
    ...
    logging.info("calced_slices: %s actual slices: %s", calced_slices, act_slices)
