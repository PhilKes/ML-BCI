...
def calc_amphase(ds_data, ds_tmin, ds_tmax, sampling_rate, ts_tmin, ts_samples):
    ...
    if ts_tmin < ds_tmin or (ts_tmin + ts_samples / sampling_rate) > ds_tmax:
        logging.info("  - Error: trial slice out of data set specific range")
...
def PSDs_plot(fname, fmin, fmax, ts_offsets):
    ...
    :return:
    """
    logging.info("    - Load time-freq. PSD map from file: %s", fname)
    ...
    logging.info("    - Data set: %s", ds_name)
    logging.info("    - PSD method: %s", psd_method)
    logging.info("    - Data set shape: %s", psdMap.shape)
    logging.info("    - Trial time slice (in seconds): %s", ts_size)
    toff_min = toff[0]
    toff_max = toff[toff.shape[0] - 1]
    logging.info("    - Trial slice offset range:%s -> %s", toff_min, toff_max)

    legend = []
    for ts_offset in ts_offsets:
        logging.info("    - Time slice offset: %s", ts_offset)
...
def calc_time_freq_FFT_PSDmap(preloaded_data, preloaded_labels, ds_name, ds_tmin, ds_tmax,
    ...
    """
    logging.info("    - Calculate time frequency PSD map for dataset: %s", mi_ds)
    ...
    for offset in ts_offsets:
        ...
        logging.info("    - Trial slice with ts_tmin = %f, num_of_samples = %d",ts_tmin, ts_samples)
    ...
    logging.info("tf_psdMap shape: %s", tf_psdMap.shape)
    ...
    while os.path.isfile(fname1):
        logging.info("file " + fname1 + " already exists")
        findex = findex + 1
        fname1 = fname + '-' + '%02d' % (findex)
        fname1 = fname1 + '.npz'
    ...
    np.savez(fname1,
             ...
             toff=ts_offsets,
             tf_psdMap=tf_psdMap)
    logging.info("  - Result saved in file %s", fname1)
    return
...
def calc_time_freq_MT_PSDmap(preloaded_data, preloaded_labels, ds_name, ds_tmin, ds_tmax,
    ...
    # logging.info("mne_ts_offsets = %s", mne_ts_offsets)
...
def plot_time_freq_PSDmap(fname, rest_ts_tmin, REST_NORM='true', LOG10='true',
    ...
    logging.info("    - Load psd map related data from file: %s", fname)
    with np.load(fname, allow_pickle=True) as data:
        ds_name = data['ds_name']  # name of the data set
    ...
    logging.info("    - Data set: %s", ds_name)
    logging.info("    - PSD method: %s", psd_method)
    logging.info("    - Data set shape: %s", psd.shape)
    logging.info("    - Trial time slice (in seconds): %s", ts_size)
    logging.info("    - Rest trial offset (in seconds): %s", rest_ts_tmin)
    toff_min = toff[0]
    toff_max = toff[toff.shape[0] - 1]
    logging.info("    - Trial slice offset range: %s -> %s", toff_min, toff_max)

    logging.info("    - f.shape = %s", f.shape)
    logging.info("    - toff.shape = %s", toff.shape)

    # Check boundaries
    if tmin < toff_min or tmax > toff_max:
        logging.info("Error: tmin or tmax are out of range! %s %s", tmin, tmax)
        sys.exit()
    if fmin < f[0] or fmax > f[f.shape[0] - 1]:
        logging.info("Error: fmin or fmax are out of range!")
        sys.exit()
    ...
    if REST_NORM == 'true':
        ...
        logging.info("    - rest_ts_tmin_ind = %s", rest_ts_tmin_ind)
    ...
    logging.info("psd_plot.shape = %s", psd_plot.shape)
...
def calc_plot_Fb_psd(fname, rest_ts_tmin, f_bands, tmin=-1.5, tmax=3.9,
    ...
    """
    # Read psd data from file
    logging.info("    - Load psd map related data from file: ", fname)
    with np.load(fname, allow_pickle=True) as data:
        ds_name = data['ds_name']  # name of the data set
        psd_method = data['psd_method']  # method used to calc. psd
        ts_size = data['ts_size']  # trial slice size used during measurement
    ...
    logging.info("    - Data set:", ds_name)
    logging.info("    - Trial time slice size (in seconds):", ts_size)
    logging.info("    - Rest trial offset (in seconds):", rest_ts_tmin)
    toff_min = toff[0]
    toff_max = toff[toff.shape[0] - 1]
    logging.info("    - Min./max. trial slice offset range:", toff_min, '->', toff_max)
    ...
    for bands in f_bands:
        fmin = bands[0]
        fmax = bands[1]
        logging.info("    - F-band: %f -> %f Hz" % (fmin, fmax))
        k = 0
        for freq in f:
    ...
    if tmin < toff[0] or tmax > toff[toff.shape[0] - 1]:
        logging.info("Error: Illegal tmin or tmax values (%f -> %f s)" % (tmin, tmax))
        sys.exit()
...
def npArray_to_mneEpochs(preloaded_data, sampling_rate, ch_names, description, tmin):
    ...
    :return:
    """
    logging.info("    - Create MNE epochs object")
    ...
    # logging.info("preloaded_data.shape =", preloaded_data.shape)
    n_channels = preloaded_data.shape[2]
    n_samples = preloaded_data.shape[3]
    ds = np.reshape(preloaded_data, (-1, n_channels, n_samples))
    # logging.info("ds.shape =", ds.shape)
...
def plot_mne_psd(preloaded_data, sampling_rate, ch_names, description, ds_tmin, ds_tmax, tmin, tmax):
    ...
    :return:
    """
    logging.info("  - Calculate and plot psd using MNE's multitaper method")

    if tmin < ds_tmin or tmax > ds_tmax:
        logging.info("ERROR: tmin or tmax are out of legal range")
        sys.exit()

    logging.info("    - Start time to consider: %2.2f s" % tmin)
    logging.info("    - End   time to consider: %2.2f s" % tmax)
...
if __name__ == '__main__':
    ...
    logging.info("EEG dataset analysis started")
    ...
    if sub_command != 'TF_plot' and sub_command != 'Fb_power' \
        ...
        logging.info("  - Name of active dataset name: ", dataset.CONSTANTS.name)
        excluded = []
        ...
        logging.info("  - Channel names: ", ch_names)
        ...
        logging.info("  - ds_tmin, ds_tmax =", ds_tmin, ds_tmax)
        ...
        logging.info("  - Shape of preloaded dataset: ", preloaded_data.shape)
        ...
        logging.info("  - Data set %s load with following following characteristic:" % mi_ds)
        logging.info("    - Trial size: %2.2f s -> %2.2f s" % (CONFIG.EEG.TMIN, CONFIG.EEG.TMAX))
        logging.info("    - Sample rate:", CONFIG.EEG.SAMPLERATE)
        logging.info("    - Number of subjects:", num_subjects)
        logging.info("    - NUmber of EEG channels:", num_channels)
        logging.info("    - Number of trials:", num_trials)
        logging.info("    - Number of samples per trial:", num_samples)

    # Sub_command jump table
    ### TF_map ##########################################################################
    if sub_command == 'TF_map':
        logging.info("  - Subcommand '%s': Calculate time-frequency psd map" % sub_command)
        ...
        if psdMethod == 'PUFFT':
            ...
            logging.info("Error: Illegal psd method chosen!")

    ### TF_plot ##########################################################################
    elif sub_command == 'TF_plot':
        logging.info("  - Subcommand '%s': Plot time-frequency map" % sub_command)
        rest_ts_tmin = -1
    ...
    ### Fb_power #########################################################################
        ...
        logging.info("  - Subcommand '%s': Calculate freq. band specific power spectral density" % sub_command)
    ...
    ### PSD01 ###########################################################################
        ...
        logging.info("  - Subcommand '%s': Calculate and plot power spectral density" % sub_command)
    ...
    ### PSD_plot ########################################################################
        ...
        """
        logging.info("  - Subcommand '%s': Plot PSDs out of time-freq. PSD map" % sub_command)
    ...
    ### AMPHASE01 ###########################################################################
        ...
        logging.info("  - Subcommand '%s': Calculate and plot amplitude and phase spectra" % sub_command)
        ...
        logging.info("  - Illegal subcommand '%s' specified" % sub_command)
