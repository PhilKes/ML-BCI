...
from app.data.datasets import TrialsDataset
from app.machine_learning.configs_results import get_trained_model_file
from app.machine_learning.models.eegnet import EEGNet
from app.util.misc import get_class_avgs


def get_class_accuracies(act_labels: np.ndarray, pred_labels: np.ndarray) -> float:
    conf_mat = confusion_matrix(act_labels, pred_labels)
    cm = conf_mat.astype('float') / conf_mat.sum(axis=1)[:, np.newaxis]
    return 100 * cm.diagonal()


def get_trials_per_class(n_class: int, act_labels: np.ndarray) -> np.ndarray:
...
def get_confusion_matrix(act_labels: np.ndarray, pred_labels: np.ndarray):
    return confusion_matrix(act_labels, pred_labels)


def get_tensorrt_model(model: torch.nn.Module, batch_size: int, chs: int, fp16: bool,
                       device=CONFIG.DEVICE) -> torch.nn.Module:
    """
    Returns EEGNet model optimized with TensorRT (fp16/32)
    :param chs: Amount of Channels in the Data
    """
    ...
    logging.info(f"Optimized EEGNet model with TensorRT (fp{'16' if fp16 else '32'})")
    return model


def gpu_warmup(device: torch.types.Device, warm_ups: int, model: torch.nn.Module, batch_size: int, chs: int,
               fp16: bool):
    """
    Run some example Inferences to warm up the GPU
    :param warm_ups: Number of warmups to execute
    :param chs: Amount of Channels in the Data
    :param fp16: Using Floating Point 16 or not
    """
...
def get_model(n_class: int, chs: int, model_path: str = None) -> torch.nn.Module:
    """
    Return EEGNet model
    pretrained state will be loaded if present
    :param chs: Amount of Channels in the Data
    :param model_path: Folder Path containing 'n_class_trained_model.pt' File
    :return:
    """
...
def preferred_device(preferred: str):
    """
    Returns preferred PyTorch device Object
    By default returns first CUDA Device if available
    otherwise returns CPU
    """
    dev = None
    if (preferred is None) or (preferred == "gpu") and torch.cuda.is_available():
        dev = "cuda:0"
    else:
        dev = "cpu"
...
def getOverlap(a: Tuple, b: Tuple) -> float:
    """
    Source: https://stackoverflow.com/a/2953979
    :param a: Tuple representing a number Interval
    :param b: Tuple representing a number Interval
    :return: Overlap of the given Intervals
    """
...
class MLRunData:
    ...
    def set_train_results(self, fold: int, fold_results: np.ndarray):
        """
        Set Fold's Train results
        :param fold_results as Array: (loss_values_train, loss_values_valid, best_model, best_epoch) see do_train()
        """
        self.epoch_losses_train[fold], self.epoch_losses_test[fold], self.best_model[fold], \
        self.best_epochs_test[fold] = fold_results

    def set_test_results(self, fold: int, test_accuracy: float, act_labels: np.ndarray, pred_labels: np.ndarray):
        """
    ...
    def best_epoch_loss_test(self, fold: int) -> float:
        """
        Returns Epoch with lowest Loss on Test Set of fold
        """
        return self.epoch_losses_test[fold][self.best_epochs_test[fold]]

    def set_best_fold(self, fold: int, act_labels: np.ndarray = None, pred_labels: np.ndarray = None):
        self.best_fold = fold
