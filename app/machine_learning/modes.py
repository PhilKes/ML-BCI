...
from app.machine_learning.configs_results import training_config_str, create_results_folders, save_config, \
    ...
    get_excluded_if_present, training_ss_result_str, save_training_results, save_training_numpy_data, \
    benchmark_config_str, benchmark_result_str, save_benchmark_results, live_sim_config_str, live_sim_result_str, \
    save_live_sim_results
from app.machine_learning.inference_training import do_train, do_test, do_benchmark, do_predict_on_samples
...
def training_cv(mi_ds: str, num_epochs: int, batch_size: int, n_classes: List[int],
    ...
    if only_fold is None:
        logging.info(f"Cross validation training with '{dataset.CONSTANTS.name}' started!")
    else:
        logging.info(f"Training of Fold {only_fold} with '{dataset.CONSTANTS.name}' started!")

    start = datetime.now()
    logging.info(training_config_str(config))
    ...
    if early_stop & (CONFIG.MI.VALIDATION_SUBJECTS > 0):
        ...
        logging.info(f"Validation Subjects: [{validation_subjects[0]}-{validation_subjects[-1]}]")
...
def do_n_class_training_cv(cv: GroupKFold, used_subjects: List[int], groups: np.ndarray, folds: int, n_class: int,
    ...
    logging.info(f"######### {n_class}Class-Classification")
    ...
    for fold in range(folds):
        ...
        if early_stop:
            ...
            logging.info(f"""Best Epoch: {run_data.best_epochs_test[fold]} with 
        ...
        if TEST_OVERFITTING:
            logging.info("## Testing on Training Dataset ##")
...
def training_ss(model_path: str, subject: int = None, num_epochs: int = CONFIG.MI.EPOCHS,
    ...
    start = datetime.now()
    logging.info(training_ss_config_str(config))
...
def benchmarking(model_path: str, name: str = None, batch_size: int = CONFIG.MI.BATCH_SIZE, n_classes: List[int] = [2],
    ...
    logging.info(benchmark_config_str(config))
    ...
    for class_idx, n_class in enumerate(n_classes):
        ...
        logging.info(f"Loading pretrained model from '{model_path} ({n_class}class)'")
...
def live_sim(model_path: str, subject: int = None, name: str = None, ch_names: List[str] = PHYS.CHANNELS,
    ...
    for class_idx, n_class in enumerate(n_classes):
        ...
        logging.info(live_sim_config_str(config))

        logging.info(f"######### {n_class}Class-Classification Live Simulation")
        logging.info(f"Loading pretrained model from '{model_path}'")
        model = get_model(n_class, len(ch_names), model_path)
        model.eval()
        ...
        #     logging.info(f"Trial {trial:02d}: {trial_correct_area_relative:.3f} (Class {trials_classes[trial]})")
        # logging.info(f"Average Correct Area per Trial: {np.average(trials_correct_areas_relative):.3f}")
